...
import { getCampaigns, deleteCampaignById } from 'apiSdk/campaigns';
import { CampaignInterface } from 'interfaces/campaign';

type ColumnType = ColumnDef<CampaignInterface, unknown>;
...
export function CampaignListPage(props: CampaignListPageProps) {
  ...
  const fetcher = useCallback(
    async () =>
      getCampaigns({
        ...
        order: params.order,
        searchTermKeys: ['name.contains', 'type.contains'],
        ...(params.filters || {}),
      }),
    [params.pageSize, params.pageNumber, params.searchTerm, params.order, params.filters],
  );

  const { data, error, isLoading, mutate } = useSWR<PaginatedInterface<CampaignInterface>>(
    () => `/campaigns?params=${JSON.stringify(params)}`,
    fetcher,
  );
  ...
  const handleView = (row: CampaignInterface) => {
    if (hasAccess('campaign', AccessOperationEnum.READ, AccessServiceEnum.PROJECT)) {
      router.push(`/campaigns/view/${row.id}`);
    }
  };
      ...
      : null,
    {
      ...
      cell: ({ row: { original: record } }: any) => (
        <Flex justifyContent="flex-end">
          <NextLink href={`/campaigns/view/${record.id}`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              mr={2}
              padding="0rem 8px"
          ...
          {hasAccess('campaign', AccessOperationEnum.UPDATE, AccessServiceEnum.PROJECT) && (
            <NextLink href={`/campaigns/edit/${record.id}`} passHref legacyBehavior>
              <Button
                onClick={(e) => e.stopPropagation()}
                mr={2}
                padding="0rem 0.5rem"
    ...
    <Box p={4} rounded="md" shadow="none">
      <Flex justifyContent="space-between" mb={4}>
        <Flex alignItems="center" gap={1}>
          ...
          <AccessInfo entity="campaign" />
        </Flex>

        {hasAccess('campaign', AccessOperationEnum.CREATE, AccessServiceEnum.PROJECT) && (
          <NextLink href={`/campaigns/create`} passHref legacyBehavior>
            <Button
              onClick={(e) => e.stopPropagation()}
              height={'2rem'}
